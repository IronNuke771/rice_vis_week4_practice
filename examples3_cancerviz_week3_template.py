...
def make_dict(table, key_col):
    ...
    return_dict = {}
    for row in table:
        filtered_row = list(row[:key_col]) + list(row[key_col+1:])
##        print(key_col,row,filtered_row)
        return_dict[row[key_col]] = filtered_row
    return return_dict
...
def merge_csv_files(cancer_csv_file, center_csv_file, joined_csv_file):
    ...
    cancer_dict = make_dict(read_csv_file(cancer_csv_file), CANCER_RISK_FIPS_COL)
    center_dict = make_dict(read_csv_file(center_csv_file),CENTER_FIPS_COL)
    
    # Compute joined table, print warning about cancer-risk FIPS codes that are not in USA map
    all_fips_from_dict = {i for i in cancer_dict if i in center_dict}
    excluded_fips_codes = {i for i in cancer_dict if i not in center_dict}
    fips_table = []
    excluded_fips_list = []
    for fips in all_fips_from_dict:
        fips_table.append([fips]+center_dict[fips]+cancer_dict[fips])

    # Write joined table
    write_csv_file(fips_table, "merged_county_cancer.csv")
